...
def feature_extraction(feature_csv):
    features = []

    with open(feature_csv, 'r') as f:
        data = csv.reader(f)
        for line in islice(data,155237,159812):
...
def obtain_dataset(dir_path):
    if dir_path[-1] == '/':
        clone_featureCSV = dir_path + 'GCJ_2gram_clone_cos.csv'  # 270000
        nonclone_featureCSV = dir_path + 'GCJ_2gram_nonclone_cos.csv'  # 279000多

    else:
        clone_featureCSV = dir_path + '/BCB_2gram_clone_cos.csv'
        nonclone_featureCSV = dir_path + '/BCB_2gram_nonclone_cos.csv'
